...
// import "./js/grid.js"
// import "./js/tile.js"
// import "./js/local_storage_manager.js"
...
const Application = ({ eventKeyDown = null }) => {
  console.log('aa')
  const inputManager = useRef(new KeyboardInputManager());
  useEffect(() => {
    if (!eventKeyDown) return;
    // Respond to direction keys
    console.log(eventKeyDown)
    const event = eventKeyDown;
    var modifiers = event?.altKey || event?.ctrlKey || event?.metaKey || event?.shiftKey || false; // todo::change
    var mapped = map[event.keyCode];

    if (!modifiers) {
      if (mapped !== undefined) {
        inputManager.current.emit("move", mapped);
      }
    }

    // R key restarts the game
    if (!modifiers && event.keyCode === 82) { // todo::change
      inputManager.current.restart.call(inputManager.current, event);
    }
  }, [eventKeyDown]);

  useEffect(() => {
    inputManager.current.bindButtonPress(".retry-button", inputManager.current.restart);
    inputManager.current.bindButtonPress(".restart-button", inputManager.current.restart);
    inputManager.current.bindButtonPress(".keep-playing-button", inputManager.current.keepPlaying);
  }, [])
